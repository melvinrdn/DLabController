...
class M2Worker(QObject):
    ...
    live_som = pyqtSignal(float, float, float)

    def __init__(self, stage_key, camera_key, positions, settle_s, comment, scan_name, averages=1, background=False, existing_scan_log=None, parent=None):
    ...
    def _save_png_with_meta(self, folder, filename, frame_u8, meta):
...
class M2LiveView(QWidget):
    ...
    def add_point(self, pos_mm, som_x_px, som_y_px):
...
class M2Tab(QWidget):
    ...
    def _launch_worker(self, background, existing_scan_log):
        p = self._last_params
        if not p:
            return
        ...
        self.prog.setMaximum(len(p["positions"]) if not background else 1)
    ...
    def _toggle_live_view(self, checked):
        if checked:
            if self._live_view is None:
                self._live_view = M2LiveView(None)
            self._live_view.reset()
